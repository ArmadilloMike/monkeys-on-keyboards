...
function shakespeare() {
    ...
    let index = 0
    statusEl.textContent = `Mode: Shakespeare - The monkeys have gained a higher intelligence - They cannot be stopped`
    mode = "shakespeare"
    clearWords()
...
stopBtn.addEventListener("click", () => {
    if (mode === "shakespeare") {
        header.textContent = "The Monkeys cannot be stopped"
    } else {
        clearInterval(intervalId)
        isRunning = false
        updateStatus()
